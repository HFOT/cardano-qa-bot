  ...
  nodes: {
    "basics-root": {
      ...
      options: [
        ...
        { label: "ステーキングの報酬は、どこから来るの?", next: "basics-reward" },
    ...
    "basics-reward": {
      type: "choice",
      label: "ステーキングの報酬は、どこから来るの?",
      keywords: ["なぜ報酬", "報酬が出る仕組み", "プルーフオブステーク", "PoS"],
      text: "**保有しているだけでは報酬は出ません。** 報酬を受け取るには、ウォレットで**ステークプールに委任(ステーキング)する**必要があります。\n\nもうひとつ、2025年のPlominハードフォーク以降は、**受け取った報酬を引き出すのにDRepへの委任も必要**になりました。DRepを選びたくない場合は「棄権(Abstain)」という既定の選択肢を選ぶこともできます。報酬の計算自体は委任した時点から始まりますが、引き出せる状態にするにはこの一手間が要ります。\n\nでは、その報酬はどこから来るのか。Cardanoは、取引の正しさを**預けられたADAの量で守る**仕組み(プルーフ・オブ・ステーク)です。新しいブロックを作る担当は、**多くのADAを託されているプールほど回ってきやすい**ように決まります。担当になったプールがブロックを作ると、その報酬が委任した人にも分配されます。\n\nよくある誤解を整理すると、\n\n・**委任してもADAはウォレットから離れません**\n・**ロックもされません。** いつでも送金できます\n・預けているのは「ADAそのもの」ではなく「**そのADAの重み**」です\n・**取引所に置いたままのADAは、自分では委任できません。** 取引所が独自のステーキングを提供している場合は別です\n\n報酬はプールの働きに左右されるので、どのプールに委任するかで結果が変わります。",
      options: [
        { label: "プールの選び方を見る", next: "spo-root" },
        { label: "いまの数字を見る", next: "basics-now" },
      ],
    },
    ...
    "basics-vote": {
      ...
      text: "**Cardanoには所有者がいません。**\n\n止められる会社もなければ、独断で仕様を変えられる人もいない。ではどうやって物事を決めるのか——**決め方そのものを決めておく**しかありません。それがガバナンスです。\n\n・**憲法**があり、何をしてよいかの枠が書かれています\n・**DRep**(代理投票者)がいて、自分の代わりに投票してくれます\n・**トレジャリーの使い道**も投票で決まります\n\n保有しているADAの量が、そのまま投票の重みになります。ただし**持っているだけでは、その力は働きません。** **DRepに委任する**か、棄権を選ぶか、あるいは自分で投票するか——どれかを自分で選ぶ必要があります。ただし**自分で投票するには、供託金を預けて自分自身をDRepとして登録し、自分に委任する**必要があります。ふつうに保有しているだけの人がそのまま議案に投票できるわけではないので、実際の選択肢はほぼ「どのDRepに委任するか」になります。何もしなければ、その分の力は使われないままです。",
      options: [
        { label: "DRepの選び方を見る", next: "drep-root" },
        { label: "いまの数字を見る", next: "basics-now" },
      ],
    },
    ...
    "basics-who": {
      ...
      text: "開発と普及は、長らく**3つの団体**が担ってきました。\n\n・**IOG**(Input Output) — 研究と開発\n・**Cardano Foundation** — 標準化と普及を担うスイスの財団\n・**EMURGO** — 事業開発\n\nただし今は、**この3団体だけで仕様変更を決められる構造ではありません。** 決めているのは団体ではなく、**手続き**です。\n\n・**ガバナンス投票** — プロトコルの変更もトレジャリーの支出も、DRep・SPO・憲法委員会の投票を経て決まります。議案の種類ごとに、誰の承認が必要かが決まっています\n・**憲法委員会**(Constitutional Committee) — 議案がCardanoの憲法に沿っているかを確認し、承認・却下を投票します\n・**予算(Treasury)の手続き** — 使い道は提案として出され、同じ投票を通ってから支出されます\n\nつまり「誰が決めるのか」の答えは、固定の名簿ではありません。**委任先を選んだあなたも、その手続きの一部**です。\n\nもうひとつの特徴が進め方です。Cardanoは**先に学術論文を書き、査読を受けてから実装する**という順番を取ってきました。速さより検証を優先する設計で、良くも悪くもここが他と違う点だとよく言われます。",
    },

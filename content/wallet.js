export default {
  nodes: {
    ...
    "wallet-staking-rewards": {
      ...
      text: "Cardanoの報酬は約5日周期の「エポック」ごとに計算され、委任してから2〜3エポック(1〜2週間程度)後に初回の報酬が届き始めます。報酬は自動でウォレット残高に加算されるので、請求などの操作は不要です。ただし2025年のPlominハードフォーク以降は、加算された報酬を引き出すにはDRepへの委任(「棄権(Abstain)」を選ぶのでも構いません)が必要です。",
      keywords: ["報酬", "エポック"],
    },
